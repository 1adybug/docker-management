...
- 其他默认、小号和大号控件通过共享兼容层按完整组件结构恢复旧密度，不能只匹配 `h-*`、`w-*` 等通用工具类。
...
当前模板通过限定到 shadcn 组件完整结构的共享兼容规则恢复：
...
圆角 Token 放在 `tailwind.config.ts`，控件密度规则放在 `app/globals.css`。不能修改 CLI 生成组件，也不能全局重定义会影响业务布局的 `h-8`、`w-8`、`h-9`、`w-9`，或使用 `.inline-flex.h-10` 这类可能命中业务元素的宽泛选择器。
